...
async function getSyncPayToken() {
  ...
  try {
    const response = await axios.post('https://api.syncpayments.com.br/api/partner/v1/auth-token', {
      client_id: SYNCPAY_PUBLIC_KEY,
      client_secret: SYNCPAY_PRIVATE_KEY
    });
    
    syncPayToken = response.data.token || response.data.access_token || response.data;
    ...
    console.error('Erro ao gerar token SyncPay:', error.response ? error.response.data : error.message);
...
app.post('/api/pay', async (req, res) => {
  ...
  try {
    ...
    console.log(`[SyncPay PIX] Solicitando Cash-in para ${name}. Valor: R$ ${pixValue} - Produto: ${title}`);
    
    // 1. Pega o token de autenticação
    const token = await getSyncPayToken();

    // 2. Chamada REAL para a API da SyncPay (usando dados do frontend)
    const response = await axios.post('https://api.syncpayments.com.br/api/partner/v1/cash-in', {
      amount: pixValue,
    ...
    console.error('Erro ao conectar com a SyncPay:', error.response ? error.response.data : error.message);
    res.status(500).json({ success: false, message: error.response?.data?.message || 'Erro ao gerar o PIX. Verifique os dados digitados.' });
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(`🔒 Chaves da SyncPay protegidas através de variáveis de ambiente (.env)`);
});
